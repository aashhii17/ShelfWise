...
class LibraryTests(TestCase):
    def setUp(self):
        self.book = Book.objects.create(title="Test Book", author="Test Author", year=2026, category="Python")
        self.member = Member.objects.create(member_id="M1001", name="Sandeep", email="sandeep@example.com")
    ...
    def test_add_book(self):
        self.client.post(reverse("add_book"), {"title": "New Book", "author": "Writer", "year": 2020, "category": "Java"})
        self.assertTrue(Book.objects.filter(title="New Book").exists())

    def test_issue_and_return_book(self):
        response = self.client.post(reverse("issue_book", args=[self.book.pk]), {"member": self.member.pk})
    ...
    def test_extend_loan(self):
        loan = Loan.objects.create(
            book=self.book,
            member=self.member,
            borrower_name=self.member.name,
            borrower_id=self.member.member_id,
    ...
    def test_cannot_issue_unavailable_book(self):
        ...
        self.client.post(reverse("issue_book", args=[self.book.pk]), {"member": self.member.pk})
